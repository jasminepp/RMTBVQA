...
class LinearWarmupAndDecay(torch.optim.lr_scheduler._LRScheduler):
    def __init__(self, optimizer, warm_up, T_max, decay_step=10, decay_rate=0.05, last_epoch=-1):
        self.warm_up = int(warm_up * T_max)
        self.T_max = T_max - self.warm_up
    ...
    def get_lr(self):
        ...
        if self.last_epoch == 0:
            ...
            # Calculate the number of decay steps since warmup ended
            decay_steps = (self.last_epoch - self.warm_up) // self.decay_step
            return [lr * (1 - self.decay_rate) ** decay_steps for lr in self.base_lrs]
